/* eslint-disable */
/**
 * Shaka Player ↔ googlevideo SABR adapter, vendored from:
 *   https://github.com/LuanRT/kira/blob/main/src/streaming/ShakaPlayerAdapter.ts
 * MIT licensed by Luan Augusto Ronchi (LuanRT). See the kira repo for the
 * original. Local adaptations:
 *   - Inlines the small set of helper functions kira imports from
 *     ./utils/helpers (asMap, headersToGenericObject, makeResponse,
 *     createRecoverableError) so this file is self-contained.
 *   - Drops the optional browser-extension proxy (checkExtension /
 *     getInjectedProxyFunction); Piped's URL rewriting handles the
 *     proxy boundary, so we always use plain fetch.
 *   - Eslint disabled because Piped's eslint config doesn't target .ts files
 *     and the upstream style differs from Piped's.
 */
...
// ─── Inlined helpers (originally in kira's @/utils/helpers) ──────────────────

function asMap<K, V>(object: Record<string, V>): Map<K, V> {
  const map = new Map<K, V>();
  for (const key of Object.keys(object)) {
    map.set(key as unknown as K, object[key]);
  }
  return map;
}

function headersToGenericObject(headers: Headers): Record<string, string> {
  const out: Record<string, string> = {};
  headers.forEach((value, key) => {
    // Edge has been observed to return header names with a leading '\n'.
    out[key.trim()] = value;
  });
  return out;
}
...
function createRecoverableError(message: string, info?: Record<string, any>) {
...
export class ShakaPlayerAdapter implements SabrPlayerAdapter {
  ...
  private async handleUmpResponse(
    ...
    // supported by all browsers. If it's not supported, fall back to
    // reading the whole segment when ready (xhr-style).
    if (!response.body) {
  ...
  private async request(
    ...
    try {
      ...
      // Piped: always plain fetch (no browser-extension proxy injection).
