/* eslint-disable */
// Vendored from FreeTube (https://github.com/FreeTubeApp/FreeTube), MIT licensed.
// Source: src/renderer/helpers/player/SabrSchemePlugin.js
//
// Local changes:
//   - `deepCopy` is replaced with the browser-built-in `structuredClone`
//     instead of importing FreeTube's `../utils` helper.
...
function fillBufferedRanges(
    ...
    if (bufferedInfo.audio.length > 0 || bufferedInfo.video.length > 0) {
        ...
        if (audioFormatsActive) {
            ...
            activeManifestVariant = manifest.variants.find(variant => {
                return (
                    variant.audio.originalId === activeVariant.originalAudioId &&
            ...
            const remainingData = new UmpReader(chunkedDataBuffer).read(part => {
                switch (part.type) {
                        ...
                        if (mediaHeaderId === undefined) {
                            ...
                            if (
                                ...
                                if (operationInputs.isInit && mediaHeader.isInitSeg) {
                                    ...
                                    mediaHeader.sequenceNumber === operationInputs.sequenceNumber
                                ) {
                                    mediaHeaderId = mediaHeader.headerId;
                                }
                            }
                    ...
                    case UMPPartId.NEXT_REQUEST_POLICY: {
                        ...
                        currentState.abrRequest.streamerContext.playbackCookie = nextRequestPolicy?.playbackCookie
                            ? PlaybackCookie.encode(nextRequestPolicy.playbackCookie).finish()
                            : undefined;
...
export function setupSabrScheme(sabrData, getPlayer, getManifest, playerWidth, playerHeight) {
    ...
    shaka.net.NetworkingEngine.registerScheme(
        "sabr",
        (uri, request, requestType, _progressUpdated, headersReceived, _config) => {
            ...
            const drcEnabled =
                url.searchParams.has("drc") || !!(activeVariant && activeVariant.audioRoles.includes("drc"));
            const enableVoiceBoost =
                url.searchParams.has("vb") || !!(activeVariant && activeVariant.audioRoles.includes("vb"));
            ...
            const requestData = {
                ...
                streamerContext: {
                    ...
                    sabrContexts,
                    unsentSabrContexts,
                    playbackCookie: sabrStreamState.nextRequestPolicy?.playbackCookie
                        ? PlaybackCookie.encode(sabrStreamState.nextRequestPolicy.playbackCookie).finish()
                        : undefined,
                },
                field1000: [],
